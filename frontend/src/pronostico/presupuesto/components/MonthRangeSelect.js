...
const MonthCell = styled(Box)(({ theme, isStart, isEnd, isInRange }) => ({
...
export default function MonthRangeSelect({ value = {}, onChange, placeholder }) {
  ...
  const isInRange = (yearMonth) => {
    if (!tempSelection.from || !tempSelection.to) return false;
    return yearMonth === tempSelection.from || 
           yearMonth === tempSelection.to || 
