-- Script para MySQL Workbench - Notificaciones con IDs autoincrementales
-- Ejecutar directamente en MySQL Workbench

-- Primero, modificar la tabla notifications para que el id sea autoincremental
ALTER TABLE notifications MODIFY COLUMN id BIGINT AUTO_INCREMENT PRIMARY KEY;
...
-- Crear notificaciones de ejemplo con diferentes tipos, severidades y fechas
-- Notificaciones de hoy (para pruebas de digest diario)
...
(1, 'REMINDER_CUSTOM', 'Recordatorio: Vencimiento de monotributo', 'No olvides pagar el monotributo antes del 20 de febrero', 'INFO', 'REMINDER', 'rem_001', false, NOW()),
(1, 'REMINDER_DEADLINE', 'Vencimiento próximo: Factura 001', 'La factura 001 vence en 3 días', 'WARN', 'REMINDER', 'rem_002', false, NOW()),
...
-- Notificaciones de ayer (para pruebas de digest diario)
INSERT INTO notifications (user_id, type, title, body, severity, resource_type, resource_id, is_read, created_at) VALUES
...
(1, 'FORECAST_ALERT', 'Alerta de pronóstico', 'El pronóstico indica una tendencia negativa', 'WARN', 'FORECAST', 'forecast_001', false, DATE_SUB(NOW(), INTERVAL 1 DAY)),
(1, 'DATA_IMPORTED', 'Datos importados exitosamente', 'Se importaron 150 movimientos desde el banco', 'INFO', 'DATA', 'data_001', true, DATE_SUB(NOW(), INTERVAL 1 DAY));
...
(1, 'MONTHLY_SUMMARY', 'Resumen mensual disponible', 'Tu resumen mensual de enero está listo', 'INFO', 'SUMMARY', 'summary_001', false, DATE_SUB(NOW(), INTERVAL 2 DAY));

-- Notificaciones de hace 3 días
INSERT INTO notifications (user_id, type, title, body, severity, resource_type, resource_id, is_read, created_at) VALUES
(1, 'REMINDER_RECURRING', 'Recordatorio recurrente', 'Recordatorio semanal: Revisar gastos', 'INFO', 'REMINDER', 'rem_003', true, DATE_SUB(NOW(), INTERVAL 3 DAY)),
(1, 'USER_INVITED', 'Usuario invitado', 'Se invitó a juan@empresa.com al proyecto', 'INFO', 'USER', 'user_001', true, DATE_SUB(NOW(), INTERVAL 3 DAY)),
(1, 'ROLE_CHANGED', 'Rol actualizado', 'Tu rol fue actualizado a "Administrador"', 'INFO', 'USER', 'user_002', true, DATE_SUB(NOW(), INTERVAL 3 DAY));

-- Notificaciones de hace 4 días
INSERT INTO notifications (user_id, type, title, body, severity, resource_type, resource_id, is_read, created_at) VALUES
(1, 'DATA_EXPORTED', 'Datos exportados', 'Se exportaron los datos del mes de enero', 'INFO', 'DATA', 'data_002', true, DATE_SUB(NOW(), INTERVAL 4 DAY)),
(1, 'REMINDER_BILL_DUE', 'Factura próxima a vencer', 'La factura 002 vence en 2 días', 'WARN', 'REMINDER', 'rem_004', false, DATE_SUB(NOW(), INTERVAL 4 DAY));
